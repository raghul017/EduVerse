...
function Dashboard() {
  ...
  return (
    <div className="min-h-screen text-textPrimary font-sans pb-24 overflow-x-hidden" style={{ backgroundColor: 'var(--page-bg-light)' }}>
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-8 pt-16">
        ...
        <ScrollReveal>
          <header className="mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            ...
            <div className="flex flex-col items-end gap-6">
               <p className="text-textSecondary text-xl max-w-md text-right text-balance">
              ...
              <div className="flex gap-4">
                 <Magnetic>
                   <button className="ev-button ev-button--secondary text-xs uppercase tracking-wider py-3 px-6">
                      <Activity size={14} className="mr-2" /> Activity Log
                   </button>
                 </Magnetic>
                 <Magnetic>
                   <button className="ev-button ev-button--primary text-xs uppercase tracking-wider py-3 px-6 font-bold">
                      <Zap size={14} className="mr-2" /> Quick Action
                   </button>
                 </Magnetic>
              </div>
            </div>
        ...
        <ScrollReveal delay={0.2} stagger={0.1}>
          <div className="bento-grid gap-6">
             ...
             <SpotlightCard variant="solid" className="col-span-1 md:col-span-2 min-h-[300px] p-8 flex flex-col justify-between overflow-hidden relative group">
                ...
                <div className="flex justify-between items-start z-10 relative">
                   <div>
                      ...
                      <div className="text-7xl font-bold text-black tracking-tighter -ml-1">
                         {stats.streak_count || 0}<span className="text-3xl text-black/40 ml-2 font-medium">days</span>
                      </div>
                   </div>
             ...
             <SpotlightCard className="p-8 relative flex flex-col items-center justify-center overflow-hidden min-h-[300px]" spotlightColor="rgba(59, 130, 246, 0.15)">
             ...
             {/* Metric Cards - Row 2 */}
             <SpotlightCard className="p-8 flex flex-col justify-between hover:bg-surface-hover transition-colors min-h-[220px]" spotlightColor="rgba(255, 255, 255, 0.08)">
                ...
                <div>
                   <div className="text-4xl font-bold text-white tracking-tight">{stats.posts_watched || 0}</div>
                   <div className="text-sm text-textMuted mt-1">Lessons Completed</div>
                </div>
             </SpotlightCard>
             ...
             <SpotlightCard className="p-8 flex flex-col justify-between hover:bg-surface-hover transition-colors min-h-[220px]" spotlightColor="rgba(255, 255, 255, 0.08)">
                ...
                <div>
                   <div className="text-4xl font-bold text-white tracking-tight">{stats.bookmarks_count || 0}</div>
                   <div className="text-sm text-textMuted mt-1">Saved Resources</div>
                </div>
             </SpotlightCard>
             ...
             <SpotlightCard className="p-8 flex flex-col justify-between hover:bg-surface-hover transition-colors min-h-[220px]" spotlightColor="rgba(255, 255, 255, 0.08)">
                ...
                <div>
                   <div className="text-4xl font-bold text-white tracking-tight">{stats.total_time || 0}<span className="text-lg text-textDisabled ml-1 font-normal">h</span></div>
                   <div className="text-sm text-textMuted mt-1">Focus Time</div>
                </div>
             </SpotlightCard>
             ...
             <div className="col-span-1 md:col-span-3 mt-8">
                <div className="flex items-center justify-between mb-8">
                   <h3 className="text-sm font-mono text-textMuted flex items-center gap-2 uppercase tracking-widest">
                      <Layers size={14} /> Active Directives
                   </h3>
                   <div className="h-[1px] flex-1 bg-border/50 ml-6"></div>
                </div>
                
                <div className="grid gap-4">
                   {stats.subjects_progress && stats.subjects_progress.length > 0 ? (
                      stats.subjects_progress.map((subject, idx) => (
                         <SpotlightCard key={idx} className="p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-6 group hover:border-accent/40" spotlightColor="rgba(161, 255, 98, 0.08)">
                            <div className="w-16 h-16 rounded-2xl bg-surface flex items-center justify-center text-accent font-bold text-2xl border border-border group-hover:border-accent/50 transition-colors shadow-lg shadow-black/20">
                               {subject.name.charAt(0)}
                            </div>
                            <div className="flex-1">
                               <div className="flex justify-between mb-3">
                                  <h4 className="text-xl font-bold text-white group-hover:text-accent transition-colors">{subject.name}</h4>
                                  <span className="font-mono text-sm text-textSecondary bg-surface px-2 py-1 rounded border border-border">{subject.progress}%</span>
                               </div>
                            ...
                            <Magnetic strength={20}>
                               <button className="w-12 h-12 rounded-full border border-border bg-surface hover:bg-white hover:text-black flex items-center justify-center text-textMuted transition-all duration-300 group/btn">
